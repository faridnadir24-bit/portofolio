...
export const Hero: React.FC<HeroProps> = ({ onNavigate, onOpenProject, onOpenCv }) => {
  return (
    <section 
      id="hero" 
      className="relative pt-32 pb-20 md:pt-44 md:pb-28 overflow-hidden bg-grid-pattern border-b border-white/5"
    >
      {/* Top subtle ambient light glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-gradient-to-b from-white/5 via-white/[0.02] to-transparent blur-3xl pointer-events-none -z-10" />

      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Availability & Location Top Bar */}
        <div className="flex flex-wrap items-center justify-between gap-3 mb-8 text-left">
        ...
        {/* Big Statement Headline (Benjamin Hoang / Framer Style) */}
        <div className="text-left space-y-6 max-w-5xl">
          <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter text-white uppercase leading-[0.95]">
            FARID NADIR <br />
            <span className="text-stone-400 font-light">AMRULLOH.</span>
          </h1>

          <p className="text-base sm:text-xl text-stone-300 max-w-3xl leading-relaxed font-normal">
            Mahasiswa Teknik Informatika yang berfokus pada <span className="text-white font-medium">rekayasa sistem cerdas (AI)</span>, <span className="text-white font-medium">telemetri IoT mikroelektronika</span>, dan <span className="text-white font-medium">arsitektur Full-Stack modern</span> untuk ketahanan energi, pangan, dan solusi berdampak nyata.
          </p>
        ...
        {/* Bento Grid Highlights (Benjamin Creative Signature Layout) */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mt-16 text-left">
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-6 pt-4 border-t border-white/5">
              ...
              <div className="bg-[#181818] p-3 rounded-2xl border border-white/5 col-span-2 sm:col-span-1">
                <span className="text-[10px] font-mono text-stone-400 block">HARDWARE</span>
                <span className="text-xs font-semibold text-white mt-1 block">ESP32 + MQTT</span>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-5 dark-card rounded-3xl p-6 sm:p-8 flex flex-col justify-between space-y-6">
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-2xl overflow-hidden border border-white/10 bg-[#1A1A1A] flex-shrink-0">
                  <img 
